...
TEST_CASE("[Vec3f] [operator/] Division operator")
{
	...
	{
		ufo::Vec3f zeroVec(0.0f, 0.0f, 0.0f);
		ufo::Vec3f result = scalar / zeroVec;
		REQUIRE(std::isinf(result.x));  // Division by zero results in Inf
		REQUIRE(std::isinf(result.y));  // Division by zero results in Inf
		REQUIRE(std::isinf(result.z));  // Division by zero results in Inf
	}
...
{
	ufo::Vec3f vec(1.0f, 2.0f, 3.0f);
	ufo::Vec3f otherVec(3.0f, 4.0f, 5.0f);
	float      scalar = 2.0f;
...
TEST_CASE("[Vec3f] [operator/=] Compound division operator")
{
	...
	ufo::Vec3f otherVec(3.0f, 2.0f, 3.0f);
	float      scalar = 2.0f;
...
TEST_CASE("[Vec3f] [dot] Dot product")
{
	...
	SECTION("Dot product of parallel vectors")
	{
		...
		REQUIRE(v1.dot(v2) == Catch::Approx(6.0f));
	}
	...
	SECTION("Dot product with negative components")
	{
		...
		REQUIRE(v1.dot(v2) == Catch::Approx(-3.0f));
	}
	...
	SECTION("Dot product of arbitrary vectors")
	{
		...
		REQUIRE(v1.dot(v2) == Catch::Approx(26.0f));
	}
}
...
TEST_CASE("[Vec3f] [norm]")
{
	...
	SECTION("Norm of a vector with equal components")
	{
		...
		REQUIRE(v.norm() == Catch::Approx(std::sqrt(3.0f)));
	}

	SECTION("Norm of a vector with negative components")
	{
		ufo::Vec3f v(-3.0f, -4.0f, -5.0f);
		REQUIRE(v.norm() == Catch::Approx(std::sqrt(50)));
	}
...
TEST_CASE("Vec3f")
{
	...
	ufo::Vec3f cross_test = v1.cross(v3);
...
TEST_CASE("[Vec3f] [normalize]")
{
	...
	SECTION("Normalize arbitrary vector")
	{
		...
		REQUIRE(v1.z == Catch::Approx(5.0f / length));
	}
}

TEST_CASE("[Vec3f] [normalized]")
{
	ufo::Vec3f v(3.0f, 4.0f, 5.0f);
...
TEST_CASE("[Vec3f] [angleTo]")
{
	SECTION("Angle between parallel vectors")
	{
		ufo::Vec3f v1(1.0f, 0.0f, 1.0f);
		ufo::Vec3f v2(2.0f, 0.0f, 2.0f);
	...
	SECTION("Angle between perpendicular vectors")
	{
		...
		REQUIRE(v1.angleTo(v2) == Catch::Approx(M_PI / 2.0f));
	}

	SECTION("Angle between anti-parallel vectors")
	{
		ufo::Vec3f v1(1.0f, 0.0f, 1.0f);
		ufo::Vec3f v2(-1.0f, 0.0f, -1.0f);
	...
	SECTION("Angle between arbitrary vectors")
	{
		ufo::Vec3f v1(1.0f, 1.0f, 1.0f);
		ufo::Vec3f v2(-1.0f, 1.0f, 1.0f);
		REQUIRE(v1.angleTo(v2) == Catch::Approx(std::acos(1.0f / 3.0f)));
	}

	SECTION("Angle between arbitrary vectors with z component")
	{
		ufo::Vec3f v1(1.0f, 0.0f, 1.0f);
		ufo::Vec3f v2(-1.0f, 1.0f, 1.0f);
		REQUIRE(v1.angleTo(v2) == Catch::Approx(M_PI / 2.0f));
...
TEST_CASE("[Vec3f][distance]")
{
	...
	SECTION("Distance between vectors with one zero component")
	{
		ufo::Vec3f v1(1.0f, 0.0f, 0.0f);
		ufo::Vec3f v2(0.0f, 1.0f, 1.0f);
		REQUIRE(v1.distance(v2) == Catch::Approx(std::sqrt(3.0f)));
	}
	...
	SECTION("Distance between arbitrary vectors")
	{
		...
		REQUIRE(v1.distance(v2) == Catch::Approx(sqrt(50.0f)));
	}
}

TEST_CASE("[Vec3f][distanceXY]")
{
	SECTION("Distance between arbitrary vectors")
	{
		ufo::Vec3f v1(1.0f, 2.0f, 3.0f);
		ufo::Vec3f v2(4.0f, 6.0f, -2.5f);
		REQUIRE(v1.distanceXY(v2) == Catch::Approx(5.0f));
	}
}
...
TEST_CASE("[Vec3f][clamp]")
{
	...
	{
		ufo::Vec3f v(0.0f, 0.0f, 0.0f);
		ufo::Vec3f minVal(-1.0f, -1.0f, -1.0f);
		ufo::Vec3f maxVal(1.0f, 1.0f, 1.0f);
		ufo::Vec3f result = v.clamp(minVal, maxVal);
		REQUIRE(result.x == Catch::Approx(0.0f));
		REQUIRE(result.y == Catch::Approx(0.0f));
		REQUIRE(result.z == Catch::Approx(0.0f));
	}
	...
	SECTION("Clamping of vector with components below minimum (const)")
	{
		...
		ufo::Vec3f const minVal(-1.0f, -1.0f, -1.0f);
		ufo::Vec3f const maxVal(1.0f, 1.0f, 1.0f);
		ufo::Vec3f       result = v.clamp(minVal, maxVal);
		REQUIRE(result.x == Catch::Approx(-1.0f));
		REQUIRE(result.y == Catch::Approx(-1.0f));
		REQUIRE(result.z == Catch::Approx(-1.0f));
...
TEST_CASE("[Vec3f][cross]")
{
	...
	SECTION("Cross product of arbitrary vectors")
	{
		...
		float dot_v1_result = v1.dot(result);
		float dot_v2_result = v2.dot(result);
		REQUIRE(dot_v1_result == Catch::Approx(0.0f));
		REQUIRE(dot_v2_result == Catch::Approx(0.0f));
	}
}

TEST_CASE("[Vec3f] [roll, pitch, yaw]")
{
	SECTION("non-const vector")
	{
		ufo::Vec3f v(-1.0f, 2.3f, 4.6f);
		REQUIRE(v.roll() == Catch::Approx(-1.0f));
		REQUIRE(v.pitch() == Catch::Approx(2.3f));
		REQUIRE(v.yaw() == Catch::Approx(4.6f));

		v.roll() = 5.0f;
		REQUIRE(v.roll() == Catch::Approx(5.0f));
		v.pitch() = 3.0f;
		REQUIRE(v.pitch() == Catch::Approx(3.0f));
		v.yaw() = 10.0f;
		REQUIRE(v.yaw() == Catch::Approx(10.0f));
	}

	SECTION("const vector")
	{
		ufo::Vec3f const v(-1.0f, 2.3f, 4.6f);
		REQUIRE(v.roll() == Catch::Approx(-1.0f));
		REQUIRE(v.pitch() == Catch::Approx(2.3f));
		REQUIRE(v.yaw() == Catch::Approx(4.6f));
	}
